...
_OFFERS_CACHE_TTL_SECONDS = 300.0
_SEARCH_SHORTLIST_LIMIT = 5
_offers_cache: dict[
    tuple[tuple[str, ...] | None, tuple[str, str, str, str, str]],
    tuple[float, list["OfferCandidate"]],
] = {}
...
@dataclass(frozen=True)
class OfferSnapshotSignature:
    offer_count: str
    latest_created_at: str
    latest_valid_to: str
    null_valid_to_count: str
    total_price_offer: str

    def cache_token(self) -> tuple[str, str, str, str, str]:
        return (
            self.offer_count,
            self.latest_created_at,
            self.latest_valid_to,
            self.null_valid_to_count,
            self.total_price_offer,
        )
...
def _load_active_offers(
    ...
    if has_direct_postgres():
        ...
        if visible_supermarket_ids is not None:
            ...
            params.append(sorted(visible_supermarket_ids))
        ...
        with get_postgres_cursor() as cursor:
            cursor.execute(
                ...
                WHERE {where_sql}
                ORDER BY o.price_offer ASC, o.supermarket_name ASC, p.name ASC
                """,
                params,
            )
        ...
        return [
            OfferCandidate(
                offer_id=str(row["id"]),
                product_id=str(row["product_id"] or row["product_row_id"]),
                ...
                valid_to=str(row["valid_to"]) if row.get("valid_to") is not None else None,
            )
            for row in rows
        ]
...
def _visible_supermarket_cache_key(
    visible_supermarket_ids: set[str] | None,
) -> tuple[str, ...] | None:
    if visible_supermarket_ids is None:
        return None
    return tuple(sorted(visible_supermarket_ids))


def _offer_snapshot_signature(
    visible_supermarket_ids: set[str] | None,
) -> OfferSnapshotSignature:
    if not has_direct_postgres():
        return OfferSnapshotSignature("0", "", "", "0", "0")
    ...
    with get_postgres_cursor() as cursor:
        cursor.execute(
            ...
            SELECT
              ...
              COALESCE(MAX(o.created_at)::text, '') AS latest_created_at,
              COALESCE(MAX(o.valid_to)::text, '') AS latest_valid_to,
              COUNT(*) FILTER (WHERE o.valid_to IS NULL)::text AS null_valid_to_count,
              COALESCE(SUM(o.price_offer)::text, '0') AS total_price_offer
            FROM public.offers o
            WHERE {where_sql}
            """,
            params,
    ...
    return OfferSnapshotSignature(
        offer_count=row.get("offer_count", "0"),
        latest_created_at=row.get("latest_created_at", ""),
        latest_valid_to=row.get("latest_valid_to", ""),
        null_valid_to_count=row.get("null_valid_to_count", "0"),
        total_price_offer=row.get("total_price_offer", "0"),
    )


def _load_active_offers_cached(
    sb: Any,
    visible_supermarket_ids: set[str] | None,
) -> list[OfferCandidate]:
    visible_cache_key = _visible_supermarket_cache_key(visible_supermarket_ids)
    signature = _offer_snapshot_signature(visible_supermarket_ids)
    cache_key = (visible_cache_key, signature.cache_token())
    cached = _offers_cache.get(cache_key)
    now = monotonic()
    if cached and now - cached[0] < _OFFERS_CACHE_TTL_SECONDS:
        return cached[1]
    offers = _load_active_offers(sb, visible_supermarket_ids)
    _offers_cache[cache_key] = (now, offers)
    return offers
...
def _search_alternative_offers(
    sb: Any,
    item_name: str,
    by_product_id: dict[str, list[OfferCandidate]],
    search_cache: dict[str, list[OfferCandidate]],
) -> list[OfferCandidate]:
    normalized_name = _normalized(item_name)
    if not normalized_name:
        return []
    cached = search_cache.get(normalized_name)
    ...
    matched_offers: list[OfferCandidate] = []
    seen_offer_ids: set[str] = set()
    for term in search_terms:
        response = sb.rpc(
            "search_products_catalog",
            {"query": term, "lim": _SEARCH_SHORTLIST_LIMIT},
        ).execute()
        for row in response.data or []:
            for offer in by_product_id.get(row["id"], []):
                if offer.offer_id in seen_offer_ids:
                    continue
                seen_offer_ids.add(offer.offer_id)
                matched_offers.append(offer)
        if len(matched_offers) >= _SEARCH_SHORTLIST_LIMIT:
            break

    search_cache[normalized_name] = matched_offers
...
def _resolve_item(
    ...
    alternatives = _search_alternative_offers(
        sb,
        item.get("name", ""),
        by_product_id,
        search_cache,
...
async def optimize(
    ...
    offers = _load_active_offers_cached(sb, visible_supermarket_ids)
    by_offer_id, by_product_id = _group_offers(offers)
    grouped: dict[str, dict[str, Any]] = {}
    search_cache: dict[str, list[OfferCandidate]] = {}

    for item in items:
        product, matched_offer = _resolve_item(
            sb,
            item,
            by_offer_id,
            by_product_id,
