-- Keep Comune-area offer discovery bounded.  The API receives only the
-- requested page after PostgreSQL has selected the nearest target for every
-- cloned offer and calculated the supermarket counters.

CREATE INDEX IF NOT EXISTS idx_offers_public_current_representative
  ON public.offers (supermarket_id, source_offer_id, name, id)
  INCLUDE (valid_from, valid_to)
  WHERE is_confirmed = true
    AND offer_kind = 'published_target';

DROP FUNCTION IF EXISTS public.nearby_public_offer_page(
  UUID[], DOUBLE PRECISION[], TEXT, TEXT, TEXT, UUID, UUID[], INTEGER, INTEGER
);

CREATE FUNCTION public.nearby_public_offer_page(
...
AS $$
  ...
  ranked_offers AS (
    ...
    WHERE offer.is_confirmed = true
      ...
      AND (
        ...
        OR offer.name ILIKE '%' || btrim(filter_query) || '%'
      )
  ...
  representatives AS (
    SELECT id, name, supermarket_id, distance_km
    FROM ranked_offers
  ...
  page AS (
    SELECT id, distance_km, row_number() OVER (
      ORDER BY COALESCE(lower(name), ''), id
    ) AS page_order
