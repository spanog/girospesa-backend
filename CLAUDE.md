...
- Draft offer persistence must stay idempotent: dedupe pending offer rows on `(product_id, flyer_id, format_key)`, then use `offers.upsert(..., on_conflict="product_id,flyer_id,format_key", ignore_duplicates=True)`. The supporting `idx_offers_product_flyer_format` unique index must not be partial; PostgREST cannot infer partial indexes for this conflict target.
