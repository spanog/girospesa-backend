...
- For multi-page PDFs, Gemini extraction must split the document into rigid 3-page PDF chunks and process one chunk per request. After each successful chunk, update `flyers.extraction_metadata` with `pages_processed`, `current_chunk_start`, `current_chunk_end`, `progress_percent`, `chunks_completed`, `chunks_total`, and `products_found`. If one chunk fails after retries, fail the whole flyer; do not salvage partial results.
...
- `/optimize` must resolve `pinned_offer_id` before fuzzy matching. A list item added from an offer is an exact offer match (`match_score=1.0`) when the offer is active and in range. When the frontend selects an optimization alternative, `PATCH /lists/{list_id}/items/{item_id}` persists the new `pinned_offer_id`, `pinned_product_id`, `found_deals`, category, and subcategory.
