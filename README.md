...
| Metodo | Path | Auth | Descrizione |
|--------|------|------|-------------|
| `GET` | `/products` | ❌ | Offerte attive con ricerca `q` ibrida (`word_similarity` + match per prefisso/sottostringa), filtro esatto `product_id` sul prodotto canonico, filtri `category`, `supermarket` (slug compat legacy) o `supermarket_id` (punto vendita esatto), ordinamento default per nome prodotto, `sort=expiry` per scadenza crescente, `expiring_soon=true` per offerte che scadono entro 3 giorni, paginazione |
