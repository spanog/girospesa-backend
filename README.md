...
Le notifiche di pubblicazione volantino sono accodate in `notification_jobs` e drenate fuori dalle richieste utente da APScheduler o da `POST /ops/cron/notifications`. Un job padre individua tutti gli admin, il manager della sede pubblicata e i customer nel raggio della loro posizione attiva (ricerca, altrimenti casa); job figli indipendenti persistono l'inbox e inviano il push solo quando l'account lo ha abilitato.
