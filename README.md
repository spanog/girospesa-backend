...
> **Estrazione AI volantini:** questo repo contiene runtime, review flow e CLI di valutazione. Il vecchio workspace di estrazione e' stato assorbito in questo backend. Quando il modello localizza un packshot nel PDF, la bozza conserva automaticamente il relativo ritaglio per la review. I PDF nuovi vengono inviati a Gemini in chunk da due pagine, generati uno alla volta: la dimensione complessiva del PDF può quindi superare il limite inline di 20 MB applicato alle immagini singole. Per contenere la RAM, il PDF originale viene scaricato in un file temporaneo tramite stream, mentre i chunk inviati al modello restano identici al documento caricato; i risultati vengono persistiti e rilasciati un chunk alla volta. I crop sono renderizzati a risoluzione 2× e ogni pagina viene renderizzata una sola volta per tutti i suoi prodotti, con un crop codificato e caricato alla volta; dopo ogni pagina il runtime rilascia le allocazioni native non più necessarie. I log dell'estrazione includono `rss_mib` corrente e `peak_rss_mib` dopo download, pagina crop e completamento, utile per verificare il margine del piano Render. La conferma pubblica le copie per filiale tramite RPC SQL set-based: contratto API, crop, notifiche e contenuti delle offerte restano invariati, senza materializzare tutti i cloni nella RAM FastAPI. Se una pagina del chunk restituisce al massimo un prodotto mentre la pagina vicina ne contiene almeno quattro, il backend la riestrae singolarmente con un controllo di copertura e conserva il risultato solo se più completo. La localizzazione del crop è persistita internamente, così dopo un riavvio vengono completate solo le immagini mancanti. I checkpoint conservano la dimensione chunk originale per una ripresa coerente.
