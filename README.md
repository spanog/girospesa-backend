...
Le notifiche di pubblicazione volantino sono accodate in `notification_jobs` e drenate fuori dalle richieste utente da APScheduler o da `POST /ops/cron/notifications`. Un job padre individua i customer nel raggio della loro posizione attiva (ricerca, altrimenti casa); job figli indipendenti persistono l'inbox e inviano il push solo quando l'account lo ha abilitato.

`NOTIFICATION_DELIVERY_WORKERS` è opzionale (default `8`, minimo `1`, massimo `16`) e limita i job figlio consegnati in parallelo.
`GET /offers` restituisce offerte pubbliche autosufficienti, incluse immagine estratta, dati del supermercato e periodo di validità. Con la posizione attiva restituisce solo offerte dei supermercati nel raggio richiesto; quando una stessa offerta di un volantino è pubblicata per più sedi, restituisce una sola copia, associata alla sede più vicina. Il filtro ripetibile `supermarket_ids` limita prima le sedi candidate. L'endpoint non espone un parametro di ordinamento: mantiene una sequenza stabile per la paginazione. `GET /supermarkets?with_active_offers=true` e `GET /flyers/public` applicano il raggio: per utenti autenticati deriva posizione e distanza dal profilo, per ospiti dal cookie di località firmato. Un token admin su `GET /supermarkets` riceve tutte le sedi attive senza filtro geografico. Per ciascun supermercato, `GET /flyers/public` ordina i volantini per scadenza più vicina. I volantini pubblici si sfogliano tramite il backend, che restituisce file e anteprima WebP senza esporre URL Storage; la preview pubblica ha cache HTTP, mentre i flussi amministrativi privati usano URL firmati di breve durata.
