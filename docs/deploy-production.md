...
| `CORS_EXTRA_ORIGINS` | Mobile/prod app | Extra allowed origins beyond `FRONTEND_URL`. | Comma-separated exact app origins: `https://girospesa.localhost,https://app.girospesa.local,capacitor://app.girospesa.local`. Never use wildcards or `http://localhost`. |
