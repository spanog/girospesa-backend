...
- Le RPC `confirm_source_flyer_offers` e `materialize_source_flyer_targets` sono eseguibili solo da `service_role`: confermano e materializzano in SQL le offerte sorgente e le copie per filiale, mantenendo idempotenza e conteggi senza trasferire il prodotto cartesiano al backend.
...
- PostGIS è abilitato nello schema `extensions` per i punti centrali ISTAT in `municipalities.center`. `user_profiles` conserva solo `municipality_code` e `max_distance_km` (1–20); `supermarkets` conserva solo `municipality_code`. La RPC service-role `visible_supermarkets_for_municipality` include sempre tutte le filiali nel Comune selezionato e aggiunge quelle esterne quando la distanza tra i centri dei Comuni è entro raggio. La RPC service-role `nearby_public_offer_page` riceve le sedi visibili con la relativa distanza, sceglie il target più vicino per ogni offerta clonata, calcola i conteggi per supermercato e restituisce una sola pagina di ID offerta.
